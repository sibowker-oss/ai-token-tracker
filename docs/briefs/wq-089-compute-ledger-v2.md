...
| D5 | **Editorial spine recast.** Current page leads on gross/net split + concentration. Corrected page leads on **bucket 1 circular financing** (frontier labs paying hyperscalers ~$45–50B, who report it as AI revenue, while labs are funded partly by hyperscaler equity and partly by selling tokens that re-route compute back). The gross/net distinction stays but is no longer the headline — it's a smaller layer of the story. | 2026-05-06 |
...
8. **Add bucket 1 as a visible page element.** New card or sub-card surfacing "Frontier lab compute spend on hyperscalers — the circular-financing line." Show 2025 lookback total (~$45–50B), per-lab attribution where defensible (OpenAI→Azure, Anthropic→AWS+GCP, Mistral→GCP+others). This becomes the editorial spine per D5.
